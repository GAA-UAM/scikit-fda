...
# Plot toy dataset
X.plot(group=y, group_colors=['C0', 'C1'])  # noqa: Q000
...
fig = X_train.plot(group=y_train, group_colors=['C0', 'C1'])  # noqa: Q000

# Plot ball
sample.plot(fig=fig, color='red', linewidth=3)  # noqa: Q000
lower = sample - radius
upper = sample + radius
...
fig = X_train[distances <= radius].plot(color='C0')  # noqa: Q000
sample.plot(fig=fig, color='red', linewidth=3)  # noqa: Q000
fig.axes[0].fill_between(
    sample.grid_points[0],
    lower.data_matrix.flatten(),
...
radius_nn = RadiusNeighborsClassifier(radius=radius, weights='distance')  # noqa: Q000
...
radius_nn.set_params(radius=0.5)  #  Radius 0.05 in the L2 distance  # noqa: E501, RUF003
radius_nn.fit(X_train, y_train)
