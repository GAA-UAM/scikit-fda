...
class FunctionalDiffusionGenerator(BaseEstimator):
    ...
    **Forward process.** Given a clean functional datum
    ...
    by the chosen
    :class:`~skfda.datasets.diffusion.diffusion_process.ForwardDiffusionProcess`.
    ...
    **Reverse process.** New samples are generated by starting from noise
    ...
    Parameters:
        diff_process: Forward diffusion process that defines
            ...
            :class:`~skfda.datasets.diffusion.diffusion_process.VariancePreservingDiffusionProcess`.
        score_model: Pre-built score network.  Must implement
            ...
            :class:`~skfda.datasets.diffusion.score_model.ScoreModelConv`
        ...
        n_jobs: Number of worker processes for
            ...
            process; ``-1`` uses all available CPU cores.
            Defaults to ``0``.
        device: Computation device for training and generation
            (e.g. ``"cpu"`` or ``"cuda"``).  Defaults to ``"cpu"``.
        random_state: Seed or random-state object for reproducible
            results.
    ...
    Examples:
        ...
        >>> from skfda.datasets.diffusion.diffusion_model import (
        ...     FunctionalDiffusionGenerator,
        ... )
        >>> X = make_gaussian_process(
        ...     n_samples=30, n_features=64, random_state=0,
        ... )
        >>> gen = FunctionalDiffusionGenerator(max_iter=5, random_state=0)
    ...
    def __init__(
        ...
        Args:
            ...
            verbose: Whether to show a tqdm progress bar during training.
                Default is True.
        """
        super().__init__()
        self.diff_process = (
    ...
    def _build_score_model(
        self, embed_dim: int, random_state: RandomStateLike = None,
    ) -> torch.nn.Module:
    ...
    def fit(
        ...
        Examples:
            ...
            FunctionalDiffusionGenerator(max_iter=5, random_state=0)
        ...
        if X.dim_domain != 1 or X.dim_codomain != 1:
            raise ValueError("FunctionalDiffusionGenerator only supports 1D functional data.")
        ...
        # One RNG for the entire fit call; sub-components draw independent
        # seeds from it, preventing correlated randomness.
        rng = validate_random_state(self.random_state)
        self.score_model_ = self._build_score_model(
            embed_dim=grid_size, random_state=rng,
        )
        ...
        # Independent generators for the DataLoader shuffle and the training loop.
        loader_generator = make_torch_generator(rng, device="cpu")
        train_generator = make_torch_generator(rng, device=self.device)
        ...
        for _ in epoch_iter:
            ...
            for x in data_loader:
                ...
                loss = self._loss_function(
                    self.score_model_, x_dev, t, train_generator,
                )
                optimizer.zero_grad()
                loss.backward()
            ...
            if self.verbose:
                epoch_iter.set_description(  # type: ignore[union-attr]
    ...
    def _loss_function(
        ...
        Args:
            ...
            generator: The torch Generator used to sample the noise.
    ...
    def generate(
        ...
        Args:
            ...
            reverse_process: Reverse-time solver used to denoise the
                ...
                :class:`~skfda.datasets.diffusion.reverse_diffusion.SDEReverseDiffusionProcess`
                with an Euler-Maruyama integrator
                :class:`~skfda.datasets.diffusion.integrators.EulerMaruyamaIntegrator`
                is used.
        ...
        if reverse_process is None:
            reverse_process = SDEReverseDiffusionProcess(
                EulerMaruyamaIntegrator(
                    random_state=rng, device=self.device,
                ),
            )
    ...
    def load_score_model(self, file_path: str | Path) -> "FunctionalDiffusionGenerator":
