...
def _compute_region(
    fdatagrid: FDataGrid,
    indices_descending_depth: np.ndarray,
    prob: float,
) -> FDataGrid:
...
def _compute_envelope(region: FDataGrid) -> Tuple[np.ndarray, np.ndarray]:
...
def _predict_outliers(
    fdatagrid: FDataGrid,
    non_outlying_threshold: Tuple[np.ndarray, np.ndarray],
) -> np.ndarray:
...
def _non_outlying_threshold(
    central_envelope: Tuple[np.ndarray, np.ndarray],
    factor: float,
