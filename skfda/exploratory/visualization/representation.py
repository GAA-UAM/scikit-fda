...
def plot_curves(fdata, chart=None, *, derivative=0, fig=None, axes=None,
    ...
    if sample_labels is not None:
        ...
        if 'color' in kwargs:
            ...
            sample_colors = np.empty((fdata.n_samples,)).astype(str)
            next_color = True
        ...
        for i in range(fdata.dim_codomain):
            for j in range(fdata.n_samples):
                if sample_labels is None and next_color:
                    sample_colors[j] = axes[i]._get_lines.get_next_color()
