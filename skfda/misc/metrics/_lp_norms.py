...
class LpNorm:
    ...
    def __init__(
        ...
        if not np.isinf(p) and p < 1:
            raise ValueError(f"p (={p}) must be equal or greater than 1.")  # noqa: EM102, TRY003

        self.p = p
        self.vector_norm = vector_norm

    def __repr__(self) -> str:
        return f"{type(self).__name__}(" f"p={self.p}, vector_norm={self.vector_norm})"  # noqa: ISC001, E501

    def __call__(self, vector: Union[NDArrayFloat, FData]) -> NDArrayFloat:  # noqa: C901, UP007
        ...
        if isinstance(vector, FDataBasis):
            ...
            raise NotImplementedError(
                f"LpNorm not implemented for type {type(vector)}",  # noqa: EM102
            )
...
def lp_norm(
    vector: Union[NDArrayFloat, FData],  # noqa: UP007
    *,
    p: float,
    vector_norm: Union[Norm[NDArrayFloat], float, None] = None,  # noqa: UP007
) -> NDArrayFloat:
