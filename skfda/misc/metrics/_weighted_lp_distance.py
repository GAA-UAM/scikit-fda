...
class WeightedLpDistance(NormInducedMetric[Union[NDArrayFloat, FData]]):
    ...
    where :math:`\| {}\cdot{} \|_p` denotes the :func:`Weighted Lp norm <lp_norm>`.
    ...
    Examples:
        ...
        >>>
        >>> distance = skfda.misc.metrics.WeightedLpDistance(p=2)
    ...
    def __init__(
        self,
        p: float,
        measure: Union[Callable[[NDArrayFloat], NDArrayFloat], NDArrayFloat,None] = None,
        vector_norm: Union[Norm[NDArrayFloat], float, None] = None,
    ) -> None:

        self.p = p
        self.vector_norm = vector_norm
        self.measure = measure if measure is not None else (lambda t: np.ones_like(t))
        norm = WeightedLpNorm(p=p, measure=measure,vector_norm=vector_norm)
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"p={self.p}, measure={self.measure} ,vector_norm={self.vector_norm})"
        )


l1_distance: Final = WeightedLpDistance(p=1)
l2_distance: Final = WeightedLpDistance(p=2)
linf_distance: Final = WeightedLpDistance(p=math.inf)
...
def lp_distance(
    ...
    *,
    ...
    r"""
    WeightedLp distance for FDataGrid objects.
    ...
    where :math:`\| {}\cdot{} \|_p` denotes the :func:`WeightedLp norm <lp_norm>`.

    Note:
        This function is a wrapper of :class:`WeightedLpDistance`, available only for
        convenience. As the parameter ``p`` is mandatory, it cannot be used
        where a fully-defined metric is required: use an instance of
        :class:`WeightedLpDistance` in those cases.
    ...
    See also:
        :class:`~skfda.misc.metrics.WeightedLpDistance`

    """  # noqa: P102
    return WeightedLpDistance(p=p,measure=measure, vector_norm=vector_norm)(fdata1, fdata2)
