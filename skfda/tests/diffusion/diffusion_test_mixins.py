...
class ForwardDiffusionFitTests:
    """Mixin verifying fit() and the _get_fit_state/_restore_fit_state contract.
    ...
    @pytest.fixture
    def unfitted_instance(self, make_process):
    ...
    def test_get_fit_state_before_fit_raises(self, unfitted_instance):
        """_get_fit_state() must raise NotFittedError before fit()."""
        with pytest.raises(NotFittedError):
            unfitted_instance._get_fit_state()

    def test_get_fit_state_contains_m(self, unfitted_instance):
        ...
        state = unfitted_instance._get_fit_state()

        assert state["M"] == CUSTOM_DIM

    # ── _restore_fit_state() contract ─────────────────────────────────────────

    def test_restore_fit_state_missing_m_raises(self, unfitted_instance):
    ...
    def test_restore_fit_state_round_trip(self, unfitted_instance, make_process):
        """State from _get_fit_state() must be restorable into a fresh instance.
...
class ForwardDiffusionCheckpointTests:
    ...
    def test_to_checkpoint_before_fit_raises(self, unfitted_instance):
        """to_checkpoint() must raise NotFittedError before fit()."""
        with pytest.raises(NotFittedError):
            unfitted_instance.to_checkpoint()

    def test_to_checkpoint_returns_required_keys(self, unfitted_instance):
    ...
    def test_to_checkpoint_class_is_concrete_type(self, unfitted_instance):
    ...
    def test_to_checkpoint_init_kwargs_matches_get_params(self, unfitted_instance):
    ...
    def test_from_checkpoint_missing_top_level_keys_raises(self, unfitted_instance):
        """from_checkpoint({}) must raise ValueError."""
        with pytest.raises(ValueError):
            ForwardDiffusionProcess.from_checkpoint({})

    def test_from_checkpoint_missing_m_in_fit_state_raises(self, unfitted_instance):
        """from_checkpoint() must raise ValueError when 'M' is absent from fit_state."""
        checkpoint = {
...
class ScalarCovarianceOperatorTests(ForwardDiffusionOperatorTests):
    ...
    def test_multiply_sigma_equals_sqrt_cov_scalar_times_vector(
        ...
        """
        sigma_t  = torch.sqrt(process._cov(t_batch)).unsqueeze(1)  # (N, 1)
    ...
    def test_multiply_cov_equals_cov_scalar_times_vector(
        ...
        _cov returns (N,) — unsqueeze(1) broadcasts over the M dimension.
        """
        cov_t    = process._cov(t_batch).unsqueeze(1)  # (N, 1)
...
class DiagonalCovarianceOperatorTests(ForwardDiffusionOperatorTests):
    ...
    def test_multiply_sigma_equals_sqrt_cov_diagonal_times_vector(
        ...
        """
        cov_diag = process._cov(t_batch)           # (N, M)
...
class ForwardDiffusionSampleLimitTests:
    ...
    @pytest.fixture
    def fitted_process(self, make_process, x_batch):
    ...
    def test_sample_before_fit_raises_error(self, unfitted_instance):
        """sample_limit_distribution must raise ValueError before fit().
    ...
    def test_generator_advances_between_calls(self, fitted_process):
    ...
    def test_same_seed_gives_identical_first_sample(self, make_process, x_batch):
        """Two freshly constructed processes with equal seeds must agree on
        their first draw.
    ...
    def test_different_seeds_give_different_first_samples(
        self, make_process, make_process_alt_seed, x_batch,
    ):
        """Two fresh processes with different seeds must produce different
        first samples.
