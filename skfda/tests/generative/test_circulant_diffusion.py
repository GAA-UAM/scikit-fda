"""Tests for CirculantSymmetricMatrixDiffusionProcess and related helpers."""
...
if TYPE_CHECKING:
    ...
    _EigenvalueCallable = Callable[[torch.Tensor], torch.Tensor]
    _CallablePair = tuple[_EigenvalueCallable, _EigenvalueCallable]
    _ProcessFactory = Callable[
        [], CirculantSymmetricMatrixDiffusionProcess,
    ]

_STAT_ATOL: float = 0.05
_PSD_THRESHOLD: float = -1e-6
...
def circulant_callables() -> _CallablePair:
    ...
    def drift_term(t: torch.Tensor) -> torch.Tensor:
        return torch.full(
            (t.shape[0], half_dim), -0.5, device=t.device,
        )

    def diffusion_term(t: torch.Tensor) -> torch.Tensor:
        return torch.full(
            (t.shape[0], half_dim), 1.0, device=t.device,
        )
...
@pytest.fixture(scope="module")
def vp_eigenvalue_callables() -> _CallablePair:
    """VP-equivalent callables; μ_k(t)² + sigma_k(t)² = 1 per eigenmode.

    At T=1 with beta_min=0.1, beta_max=10.0:
        ∫₀¹ d_t dt = -2.525, μ_k(1) ≈ 0.080, sigma_k(1) ≈ 0.997
    Limit distribution ≈ N(0, 0.994·I_M); systematic deviation from
    N(0,I) is 0.003, within atol=0.05.
    """
...
@pytest.fixture(scope="module")
def constant_row_callables() -> _CallablePair:
    """Row-convention callables for the same operator as circulant_callables.

    c_0 = -0.5 (drift), c_0 = 1.0 (diffusion), all other entries zero.
    FFT([c_0, 0, …, 0])[k] = c_0 for all k — same eigenvalue spectrum
    as circulant_callables.
    """
    half_dim = DATA_DIM // 2 + 1  # 9 for DATA_DIM=16
...
@pytest.fixture(scope="module")
def p_fourier(
    circulant_callables: _CallablePair,
) -> CirculantSymmetricMatrixDiffusionProcess:
    """Fitted circulant process; fourier_drift=fourier_diffusion=True."""
...
@pytest.fixture(scope="module")
def p_row(
    constant_row_callables: _CallablePair,
) -> CirculantSymmetricMatrixDiffusionProcess:
    """CirculantSymmetricMatrixDiffusionProcess; fourier_drift=False.

    Same operator as p_fourier: FFT([c_0, 0, …, 0]).real = c_0·ones(M).
    fourier_diffusion=False exercises
    _duplicate_symmetric_row_and_get_eigenvalues for the diffusion
    callable, making multiply_sigma and multiply_cov tests meaningful as
...
def _make_fitted_constant_process(
    ...
    """Construct and fit a process with D=-0.5, g=1.0."""
...
def _circulant_drift_broadcast(t: torch.Tensor) -> torch.Tensor:
    """Constant drift eigenvalue -0.5, broadcast shape (N, 1)."""
...
def _circulant_diffusion_broadcast(t: torch.Tensor) -> torch.Tensor:
    """Constant diffusion eigenvalue 1.0, broadcast shape (N, 1)."""
...
class TestCirculantFit(
    ForwardDiffusionFitTests,
    ForwardDiffusionCheckpointTests,
):
    """Tests for fit / checkpoint of CirculantSymmetricMatrixDiffusionProcess.

    Extends the base contract:
    (1) fit() builds an internal DiagonalDiffusionProcess in eigenspace
        (diagonal_process_).
    ...
    _restore_fit_state() re-runs fit() with a dummy zero tensor, so
    round-trip tests use allclose rather than torch.equal — precomputed
    grids are rebuilt via an independent FFT path.
    """

    @pytest.fixture
    def make_process(self) -> _ProcessFactory:
        """Factory with picklable broadcast (N, 1) callables."""
    ...
    @pytest.fixture
    def make_process_with_lambda(self) -> _ProcessFactory:
        ...
        def factory() -> CirculantSymmetricMatrixDiffusionProcess:
            return CirculantSymmetricMatrixDiffusionProcess(
                drift_term=lambda t: torch.full(
                    (t.shape[0], half_dim), -0.5,
                ),
                diffusion_term=lambda t: torch.full(
                    (t.shape[0], half_dim), 1.0,
                ),
                fourier_drift=True,
                fourier_diffusion=True,
            )
    ...
    def test_get_fit_state_contains_device_key(
        self,
        fitted_instance: CirculantSymmetricMatrixDiffusionProcess,
    ) -> None:
        """_get_fit_state() must contain 'device'.

        VP and VE do not store device; this pins the Circulant-specific
        extension. Dropping 'device' would make _restore_fit_state()
        raise ValueError on any load.
        """
        state = fitted_instance._get_fit_state()  # noqa: SLF001

        assert "device" in state

    def test_fit_creates_fitted_diagonal_process(
        self,
        fitted_instance: CirculantSymmetricMatrixDiffusionProcess,
    ) -> None:
        """fit() must create a DiagonalDiffusionProcess at diagonal_process_.

        All operator arithmetic is delegated to diagonal_process_ in
        eigenspace. M == DATA_DIM confirms it was fitted on correct-
        dimension data.
        """
        assert isinstance(
            fitted_instance.diagonal_process_, DiagonalDiffusionProcess,
        )
        assert fitted_instance.diagonal_process_.M_ == DATA_DIM

    def test_fit_q_mat_has_correct_shape(
        self,
        fitted_instance: CirculantSymmetricMatrixDiffusionProcess,
    ) -> None:
        """After fit(), q_mat_ must have shape (M, M).

        An incorrect shape — e.g., (M//2+1, M) from a missing duplication
        step — would corrupt every batched matrix product through silent
        broadcasting.
        """
        assert fitted_instance.q_mat_.shape == (DATA_DIM, DATA_DIM)

    def test_fit_q_mat_is_orthogonal(
        self,
        fitted_instance: CirculantSymmetricMatrixDiffusionProcess,
    ) -> None:
        """After fit(), q_mat_ must satisfy Q @ Q.T ≈ I and Q.T @ Q ≈ I.

        Both products checked independently so a one-sided failure is
        caught. atol=1e-5 accommodates float32 rounding; a normalization
        bug produces deviations of order 1 - 1/M ≈ 0.94.
        """
        q = fitted_instance.q_mat_
        eye = torch.eye(DATA_DIM, device=q.device)

        assert torch.allclose(q @ q.T, eye, atol=1e-5), (
            "q @ q.T must equal I_M; orthogonality violated in left product"
        )
        assert torch.allclose(q.T @ q, eye, atol=1e-5), (
            "q.T @ q must equal I_M; orthogonality violated in right product"
        )

    def test_to_checkpoint_picklable_callables_no_warning(
        self,
        make_process: _ProcessFactory,
    ) -> None:
        """to_checkpoint() must not warn with picklable callables."""
    ...
    def test_to_checkpoint_nonpicklable_callable_warns_and_replaces_with_none(
        self,
        make_process_with_lambda: _ProcessFactory,
    ) -> None:
        ...
        with pytest.warns(UserWarning, match=r"not picklable"):
            checkpoint = instance.to_checkpoint()
    ...
    def test_checkpoint_round_trip(
        ...
        """Checkpoint round-trip must restore M and operator output.

        allclose (not torch.equal): _restore_fit_state() re-runs
        self.fit() with a dummy zero tensor; grids are rebuilt via
        independent FFT calls, so outputs are mathematically equal but
    ...
    def test_restore_fit_state_missing_device_raises(
        ...
        """_restore_fit_state() must raise ValueError if 'device' absent."""
        gen_state = torch.Generator().get_state()
        with pytest.raises(ValueError, match=r"device"):
            unfitted_instance._restore_fit_state(  # noqa: SLF001
                {"M": CUSTOM_DIM, "generator_state": gen_state},
            )
    ...
    def test_restore_fit_state_device_as_string_raises(
        ...
        """_restore_fit_state() must raise ValueError for string 'device'.

        Common source: JSON round-trip converting torch.device to its
        string repr.
        """
        gen_state = torch.Generator().get_state()
        with pytest.raises(ValueError, match=r"device"):
    ...
    def test_restore_fit_state_device_unsupported_type_raises(
        ...
        Passes isinstance(torch.device) but fails the ("cpu", "cuda")
        whitelist.
        """
        gen_state = torch.Generator().get_state()
        with pytest.raises(ValueError, match=r"device"):
    ...
    def test_restore_fit_state_none_callable_raises(
        ...
        Failure mode: from_checkpoint() called without a pre-built
        instance on a checkpoint saved with non-picklable callables.
        """
        instance = make_process_with_lambda()
        instance.fit(torch.randn(BATCH_SIZE, CUSTOM_DIM))
        ...
        with pytest.raises(ValueError, match=r"non-picklable"):
...
class TestCirculantOperators(ForwardDiffusionOperatorTests):
    """Operator contracts; overrides both mixin tests with atol=1e-5.

    The circulant process adds one extra _fft_to_eigenspace/_eigenspace_to_fft
    roundtrip absent in VP and diagonal implementations, contributing
    ~1e-6 per element. In the inv-sigma round-trip, this error is further
    amplified by 1/sqrt(_cov) ≈ 3.2x.
    """
    ...
    def process(
        ...
        """Fitted constant-eigenvalue process; _cov ∈ [0.095, 0.551]."""
    ...
    def test_multiply_cov_equals_multiply_sigma_applied_twice(
        ...
        """multiply_cov(h, t) must equal multiply_sigma applied twice.

        atol=1e-5 replaces the default 1e-8: the RHS contains one extra
        _fft_to_eigenspace(_eigenspace_to_fft(·)) roundtrip absent in LHS.
        """
    ...
    def test_sigma_inv_sigma_round_trip(
        ...
        atol=1e-5 replaces the default 1e-8: extra FFT roundtrip errors
        amplified by 1/sqrt(_cov) inside multiply_inv_sigma. t_batch
        interior times keep sqrt(_cov) well above _PSEUDOINV_THRESHOLD.
        """
...
class TestCirculantSampleLimit(ForwardDiffusionSampleLimitTests):
    ...
    Two additional statistical tests use orthogonal oracles: the VP
    identity μ_k(T)² + sigma_k(T)² = 1, and the per-dimension variance
    from _cov(T=1).
    """
    ...
    def make_process(
        self,
        vp_eigenvalue_callables: _CallablePair,
    ) -> _ProcessFactory:
        """Factory using VP-equivalent callables; limit ≈ N(0, I_M)."""
    ...
    def make_process_alt_seed(
        ...
        """Factory using circulant_callables; limit ≈ N(0, c_Y(1)·I_M)."""
    ...
    def test_limit_distribution_is_standard_normal_for_vp_equivalent_callables(
        self,
        fitted_process: ForwardDiffusionProcess,
    ) -> None:
        """The limit distribution must be approximately N(0, I_M) for VP.

        Derivation: d_t = -0.5 b(t), diffusion = sqrt(b(t)) satisfy
        μ_k(t)² + sigma_k(t)² = 1. At T=1: μ_k(1) ≈ 0.080,
        sigma_k(1) ≈ 0.997; systematic deviation from 1.0 is 0.003.

        atol=0.05 (n=10 000): sampling noise SE ≈ 0.007 → combined
        |std - 1.0| ≈ 0.010.
        """
        n = 10_000
        samples = fitted_process.sample_limit_distribution(n_samples=n)

        max_mean = samples.mean(dim=0).abs().max()
        assert max_mean < _STAT_ATOL, (
            f"Per-dimension mean too large: {max_mean:.4f}"
        )
        max_std_dev = (samples.std(dim=0) - 1.0).abs().max()
        assert max_std_dev < _STAT_ATOL, (
            f"Per-dimension std deviates from oracle ≈ 1.0: {max_std_dev:.4f}"
        )
    ...
    def test_limit_distribution_statistics(
        ...
        """The limit distribution must match per-dim variance from _cov(T=1).

        With D=-0.5, g=1.0: c_Y(1) = 1 - exp(-1) ≈ 0.632;
        expected_std ≈ 0.795. Oracle read from process._cov(T=1) so the
        test stays correct if callables change.

        atol=0.05 (n=10 000): mean SE ≈ 0.008 (6sigma), std SE ≈ 0.006.
        """
        n = 10_000
        ...
        t_end = torch.ones(1)
        cov_matrix = process._cov(t_end)  # noqa: SLF001
        ...
        max_std_err = (samples.std(dim=0) - expected_std).abs().max()
        assert max_std_err < _STAT_ATOL, (
...
class TestCirculantCov:
    """Tests for _cov(t); returns shape (N, M, M) per sample.

    Float32 tolerances for Q diag(c_Y) Q^T with ||Q Q^T - I||_F <= 1e-5:
        symmetry: atol=1e-6; diagonal consistency: atol=1e-6 (on-diag),
        1e-5 (off-diag); eigenspace invariant: atol=1e-5.
    """
    ...
    def process(
        ...
        """Fitted constant-eigenvalue; _cov(t) = (1 - exp(-t)) · I_M."""
        return _make_fitted_constant_process(x_batch, circulant_callables)

    def test_cov_shape_is_n_m_m(
        self,
        process: CirculantSymmetricMatrixDiffusionProcess,
        t_batch: torch.Tensor,
    ) -> None:
    ...
    def test_cov_at_t0_is_zero_matrix(
        self,
        process: CirculantSymmetricMatrixDiffusionProcess,
        t_zero: torch.Tensor,
    ) -> None:
    ...
    def test_cov_is_symmetric(
        ...
        atol=1e-6: |cov[i,j] - cov[j,i]| <= 2·sqrt(M)·e32·c_max ≈ 5e-7.
        """
        cov = process._cov(t_batch)  # noqa: SLF001

        assert torch.allclose(cov, cov.transpose(-2, -1), atol=1e-6)

    def test_cov_is_positive_semidefinite(
        self,
        process: CirculantSymmetricMatrixDiffusionProcess,
        t_batch: torch.Tensor,
    ) -> None:
        ...
        _PSD_THRESHOLD (not 0) accommodates float32 rounding.
        """
        eigvals = torch.linalg.eigvalsh(
            process._cov(t_batch),  # noqa: SLF001
        )

        assert eigvals.min() >= _PSD_THRESHOLD, (
    ...
    def test_cov_diagonal_equals_diagonal_process_cov_for_uniform_eigenvalues(
        ...
        """Main diagonal of _cov(t) must equal diagonal_process._cov(t).

        For uniform c_Y: (Q diag(c_Y) Q^T)[i,i] = c_Y·(Q Q^T)[i,i] = c_Y.
        atol=1e-6 (on-diag): error <= c_Y · sqrt(M) · e32 ≈ 3e-7.
        atol=1e-5 (off-diag): off-diag <= c_Y · ||Q Q^T - I||_F / sqrt(M).
        """
        cov = process._cov(t_batch)  # noqa: SLF001  # (N, M, M)
        cov_diag = cov.diagonal(dim1=-2, dim2=-1)  # (N, M)

        diag_ref = process.diagonal_process_._cov(t_batch)  # noqa: SLF001
        diag_err = (cov_diag - diag_ref).abs().max().item()
        assert torch.allclose(cov_diag, diag_ref, atol=1e-6), (
            f"Diagonal of _cov disagrees with diagonal_process._cov: "
            f"max error = {diag_err:.2e}"
        )
        ...
        assert torch.allclose(
            ...
            f"max |off-diag| = {off_max:.2e}"
        )

    def test_cov_in_eigenspace_is_diagonal(
        self,
        process: CirculantSymmetricMatrixDiffusionProcess,
        t_batch: torch.Tensor,
    ) -> None:
        """Q^T _cov(t) Q must equal diag(c_Y(t)); algebraic invariant.
        ...
        off_max = cov_y_offdiag.abs().max().item()
        assert torch.allclose(
        ...
        diag_ref = process.diagonal_process_._cov(t_batch)  # noqa: SLF001
        diag_err = (cov_y_diag - diag_ref).abs().max().item()
        assert torch.allclose(cov_y_diag, diag_ref, atol=1e-5), (
...
class TestCirculantDrift:
    """Tests for drift(x, t) = Q diag(λ(t)) Qt x via the FFT shortcut.

    Five tests in escalating specificity: shape, linearity, oracle
    (uniform eigenvalues), FFT-vs-matmul equivalence, and fourier/row
    ...
    def process(
        ...
        """Fitted constant-eigenvalue; oracle: drift(x, t) = -0.5 · x."""
        return _make_fitted_constant_process(x_batch, circulant_callables)
    ...
    def test_drift_is_linear_in_x(
        self,
        process: CirculantSymmetricMatrixDiffusionProcess,
        x_batch: torch.Tensor,
        t_batch: torch.Tensor,
    ) -> None:
        """drift(a*x, t) must equal a*drift(x, t) for any scalar a.

        a=3.7 (non-integer) rules out a pass linear only for integer
        multiples.
        """
        alpha = 3.7
        ...
        assert torch.allclose(
            result_scaled, result_then_scaled, atol=1e-5, rtol=1e-8,
        )
    ...
    def test_drift_with_uniform_eigenvalues_equals_scalar_times_x(
        ...
        Q diag(c) Qt x = c · Q Qt x ≈ c · x by Q-orthogonality.
        atol=1e-5: |c| · ||Q Qt - I|| · ||x||∞/sqrt(M) ≈ 3.75e-6.
        """
        result = process.drift(x_batch, t_batch)
    ...
    def test_drift_fft_path_matches_explicit_matrix_vector_product(
        ...
        """drift(x, t) must equal the explicit Q diag(λ(t)) Qt x computation.
    ...
    def test_drift_row_and_fourier_inputs_give_same_result(
        ...
        """Drift must agree for fourier_drift=True and fourier_drift=False.

        atol=1e-4: the row path applies one extra FFT to convert the
        generating sequence to eigenvalues, adding ~1e-6 on top of the
        shared drift FFT operations.
        """
...
class TestCirculantDiffusionMethods:
    """Tests for diffusion(t), diffusion_times_v(v, t), diffusion_gram_times_v.

    diffusion(t)                = Q diag(g(t)) Qt         → (N, M, M)
    diffusion_times_v(v, t)     = Q diag(g(t)) Qt v       → (N, M)
    diffusion_gram_times_v(v,t) = Q diag(g(t)²) Qt v      → (N, M)

    Tolerances: symmetry atol=1e-6; diffusion_times_v vs G@v atol=1e-5;
    diffusion_gram_times_v vs applied-twice atol=1e-5.
    """
    ...
    def process(
        ...
        """Fitted constant-eigenvalue process; g=1.0 for all eigenmodes."""
        return _make_fitted_constant_process(x_batch, circulant_callables)
    ...
    def test_diffusion_is_symmetric(
        ...
        atol=1e-6: |G[i,j] - G[j,i]| <= 2·sqrt(M)·e32·g_max ≈ 9.6e-7.
        """
        g = process.diffusion(t_batch)

        assert torch.allclose(g, g.transpose(-2, -1), atol=1e-6)

    def test_diffusion_before_fit_raises_error(
        self,
        circulant_callables: _CallablePair,
        t_batch: torch.Tensor,
    ) -> None:
    ...
    def test_diffusion_times_v_output_shape(
        self,
        process: CirculantSymmetricMatrixDiffusionProcess,
        x_batch: torch.Tensor,
        t_batch: torch.Tensor,
    ...
    def test_diffusion_gram_times_v_output_shape(
        self,
        process: CirculantSymmetricMatrixDiffusionProcess,
        x_batch: torch.Tensor,
        t_batch: torch.Tensor,
    ...
    def test_diffusion_gram_times_v_equals_diffusion_times_v_applied_twice(
        ...
        """diffusion_gram_times_v(v, t) must equal diffusion_times_v twice.

        G is symmetric, so G G^T = G². The implementation squares
        eigenvalues (g²) directly rather than composing two applications.
        atol=1e-5: RHS has one extra Q^T·Q roundtrip absent in LHS.
        """
...
class TestCirculantMeanCond:
    """Tests for mean_cond(x, t) = Q E[Y(t) | Y(0) = Qt x] via IRFFT."""
    ...
    def process(
        ...
        """Constant-eigenvalue process; oracle: mean_cond = exp(-0.5t)·x."""
        return _make_fitted_constant_process(x_batch, circulant_callables)

    @pytest.fixture
    def x_ones(self, t_sweep: torch.Tensor) -> torch.Tensor:
        """All-ones input for t_sweep; oracle reduces to scalar factor."""
    ...
    def test_mean_cond_at_t0_equals_x(
        ...
        atol=1e-5: error from FFT round-trip (Q Q^T ≈ I up to 1e-5)
        plus interpolation rounding at the precomputed grid's left boundary.
        """
    ...
    def test_mean_cond_is_linear_in_x(
        ...
        """mean_cond(a*x, t) must equal a*mean_cond(x, t).

        a=3.7 (non-integer) rules out a pass that is linear only for a=1.
        """
        alpha = 3.7
        ...
        assert torch.allclose(
            result_scaled, result_then_scaled, atol=1e-5, rtol=1e-10,
        )

    def test_mean_cond_uniform_eigenvalues_oracle(
        self,
        process: CirculantSymmetricMatrixDiffusionProcess,
        t_sweep: torch.Tensor,
        x_ones: torch.Tensor,
    ) -> None:
        """With all eigenvalues = -0.5 and x = ones, mean_cond = exp(-0.5 t).

        ∫₀ᵗ D du = -0.5t; Q diag(exp(-0.5t)) Qt·ones = exp(-0.5t)·ones.
        atol=1e-4: dominant error from trapezoid integration (~1e-4, n=1000).
        """
        expected = (
    ...
    def test_mean_cond_decays_monotonically_for_mean_reverting_eigenvalues(
        ...
        +1e-6 tolerance absorbs float rounding without masking genuine
        non-monotone behaviour (a sign flip produces steps ~1e-3).
        """
        result = process.mean_cond(x_ones, t_sweep)  # (N_sweep, DATA_DIM)
        norms = result.norm(dim=1)  # (N_sweep,)

        max_upward = (norms[1:] - norms[:-1]).clamp(min=0).max().item()
        assert (norms[1:] <= norms[:-1] + 1e-6).all(), (
            f"mean_cond norm is not monotonically non-increasing: "
            f"max upward step = {max_upward:.2e}"
        )
...
class TestFourierVsRowInputModes:
    """Tests that fourier_*=True and fourier_*=False produce identical output.

    mean_cond exercises the D-integral path independently of multiply_sigma
    and multiply_cov (F-integral path). Testing all three ensures neither
    _precompute_D_integral nor _precompute_F_integral silently picks the
    wrong branch.

    atol=1e-4: the row path introduces an extra FFT (~1e-6 per eigenvalue),
    well below the ~1e-4 trapezoid residual shared by both paths.
    """

    def test_mean_cond_equivalence(
        self,
        p_fourier: CirculantSymmetricMatrixDiffusionProcess,
        p_row: CirculantSymmetricMatrixDiffusionProcess,
        x_batch: torch.Tensor,
    ...
    def test_multiply_cov_equivalence(
        ...
        """multiply_cov must agree for fourier_diffusion=True and =False."""
...
class TestGetCosineBasis:
    ...
    Covers shape (even/odd M), orthogonality (left/right products,
    even/odd M), and column invariants (DC = 1/sqrt(M), Nyquist column).
    """

    def test_shape_even_m(self) -> None:
        """_get_cosine_basis(DATA_DIM) must have shape (DATA_DIM, DATA_DIM)."""
        q = _get_cosine_basis(DATA_DIM)

        assert q.shape == (DATA_DIM, DATA_DIM)

    def test_shape_odd_m(self) -> None:
    ...
    def test_q_qt_equals_identity_even_m(self) -> None:
        """Q @ Q.T must equal I_M for even M = DATA_DIM.

        atol=1e-5: |error| <= sqrt(M)·e32 ≈ 4.8e-7; normalization bug ≈ 0.94.
        """
        q = _get_cosine_basis(DATA_DIM)
    ...
    def test_qt_q_equals_identity_even_m(self) -> None:
        """Q.T @ Q must equal I_M for even M = DATA_DIM.

        Tested independently because a transposition error satisfies one
        product while violating the other.
        """
        q = _get_cosine_basis(DATA_DIM)
        eye = torch.eye(DATA_DIM)
    ...
    def test_orthogonality_odd_m(self) -> None:
        """Both Q @ Q.T and Q.T @ Q must equal I_9 for odd M = 9.

        atol=1e-5: for M=9, |error| <= sqrt(9) · e32 ≈ 3.6e-7.
        """
        q = _get_cosine_basis(9)
        eye = torch.eye(9)

        assert torch.allclose(q @ q.T, eye, atol=1e-5), (
            "q @ q.T != I_9: orthogonality violated for odd M"
        )
        assert torch.allclose(q.T @ q, eye, atol=1e-5), (
            "q.T @ q != I_9: orthogonality violated for odd M"
        )

    def test_first_column_is_constant_dc(self) -> None:
        """Q[:, 0] must be a constant vector with value 1/sqrt(DATA_DIM).

        atol=1e-6: single float32 division; rounding <= e32/2 ≈ 6e-8.
        """
        q = _get_cosine_basis(DATA_DIM)
    ...
    def test_nyquist_column_is_alternating_for_even_m(self) -> None:
        """Q[:, DATA_DIM//2] must equal cos(pi*i) / sqrt(DATA_DIM).

        The Nyquist frequency k = M/2 produces (+1, -1, +1, ...)/sqrt(M)
        = cos(pi*i)/sqrt(M). Present only for even M.
        atol=1e-6: two float32 operations; rounding ≈ 2.4e-7.
        """
        q = _get_cosine_basis(DATA_DIM)
...
class TestFftEigenspaceRoundTrip:
    ...
    Both must form exact round-trips and agree element-by-element with
    explicit matmuls.

    atol=1e-5: each FFT/IFFT pair contributes <= log₂(M)·e32·||x||∞ ≈ 1.4e-6.
    """

    def test_eigenspace_then_back_is_identity(
        self,
        x_batch: torch.Tensor,
    ) -> None:
    ...
    def test_back_then_eigenspace_is_identity(
        self,
        x_batch: torch.Tensor,
    ) -> None:
        ...
        one composition can be correct while the other is off by factor M.
        """
        result = _fft_to_eigenspace(_eigenspace_to_fft(x_batch))

        assert torch.allclose(result, x_batch, atol=1e-5)

    def test__fft_to_eigenspace_matches_q_matmul(
        self,
        x_batch: torch.Tensor,
    ) -> None:
    ...
    def test__eigenspace_to_fft_matches_q_t_matmul(self) -> None:
    ...
    def test_parseval_norm_preservation(
        self,
        x_batch: torch.Tensor,
    ) -> None:
        """||_fft_to_eigenspace(x)[n]||₂ must equal ||x[n]||₂ for every n.
...
class TestDuplicateSymmetricHelpers:
    """Isolation tests for the symmetric eigenvalue duplication helpers.

    Tests _duplicate_symmetric_eigenvalues and
    _duplicate_symmetric_row_and_get_eigenvalues in isolation from fit().
    """

    def test_duplicate_eigenvalues_output_shape_even_m(
        self,
        t_batch: torch.Tensor,
    ) -> None:
        """_duplicate_symmetric_eigenvalues must return (N, M) for even M."""
        m = DATA_DIM  # 16
        half_dim = m // 2 + 1  # 9

        def lambda_half(t: torch.Tensor) -> torch.Tensor:
            return torch.full((t.shape[0], half_dim), -0.5)
    ...
    def test_duplicate_eigenvalues_output_shape_odd_m(
        self,
        t_batch: torch.Tensor,
    ) -> None:
        """_duplicate_symmetric_eigenvalues must return (N, M) for odd M=9.
    ...
    def test_duplicate_eigenvalues_symmetry_property(
        self,
        t_batch: torch.Tensor,
    ) -> None:
        """Duplicated spectrum must satisfy lambdas[:, k] == lambdas[:, M-k].

        Each position in the half-spectrum is distinct (value j+1 at
        position j) so a wrong flip boundary produces a value mismatch.
        torch.equal: duplicated values are exact copies — no arithmetic.
        """
        m = DATA_DIM  # 16
        half_dim = m // 2 + 1  # 9
        ...
        def lambda_half(t: torch.Tensor) -> torch.Tensor:
            return values.unsqueeze(0).expand(t.shape[0], -1)
    ...
    def test_duplicate_row_output_shape_even_and_odd_m(
        self,
        t_batch: torch.Tensor,
        m: int,
    ) -> None:
        """_duplicate_symmetric_row_and_get_eigenvalues must return (N, M)."""
        half_dim = m // 2 + 1

        def c_half(t: torch.Tensor) -> torch.Tensor:
            return torch.full((t.shape[0], half_dim), 1.0)
    ...
    def test_duplicate_row_real_valued(
        self,
        t_batch: torch.Tensor,
    ) -> None:
        """_duplicate_symmetric_row_and_get_eigenvalues must return float32.

        The implementation returns torch.fft.fft(row).real; a missing
        .real suffix yields complex64, causing dtype errors downstream.
        """
        m = DATA_DIM  # 16
        half_dim = m // 2 + 1  # 9
    ...
    def test_duplicate_row_matches_explicit_fft(
        self,
        t_batch: torch.Tensor,
    ) -> None:
        ...
        values = torch.arange(1, half_dim + 1, dtype=torch.float32)
        c_half_val = values.unsqueeze(0).expand(BATCH_SIZE, -1)  # (N, 9)
        ...
        symmetric_row = torch.cat(
            [
                c_half_val,
                torch.flip(c_half_val[:, 1:(m + 1) // 2], dims=[1]),
            ],
            dim=1,
        )  # (BATCH_SIZE, 16)
    ...
    def test_uniform_row_gives_uniform_eigenvalues(
        self,
        t_batch: torch.Tensor,
    ) -> None:
        """[c_0, 0, …, 0] half-row must produce all M eigenvalues equal to c_0.

        FFT([c_0, 0, …, 0])[k] = c_0 for all k. Bridges the two input
        modes: the sparse-row convention is equivalent to the constant-
        eigenvalue convention.
        atol=1e-5: FFT rounding <= log₂(16) · e32 · |c_0| ≈ 2.4e-7.
        """
        m = DATA_DIM  # 16
        half_dim = m // 2 + 1  # 9
        ...
        assert torch.allclose(
            result, torch.full_like(result, -0.5), atol=1e-5,
        )
...
class TestCirculantDeviceConsistency:
    """Tests that device is propagated from training data.

    Two device-sensitive components: q_mat (built with device from
    _get_cosine_basis) and diagonal_process (fitted on
    _fft_to_eigenspace(x), inheriting x.device).

    Both cpu_process and cuda_process are class-scoped. Each generates
    its own training data inline — pytest forbids class-scoped fixtures
    from depending on function-scoped conftest fixtures. Callables must
    use device=t.device so the same callables work on both devices.
    """
    ...
    def cpu_process(
        ...
        """CirculantSymmetricMatrixDiffusionProcess fitted on CPU data."""
        torch.manual_seed(SEED)
    ...
    def cuda_process(
        ...
        """CirculantSymmetricMatrixDiffusionProcess fitted on CUDA data."""
    ...
    def test_device_attribute_is_cpu_after_cpu_fit(
        self,
        cpu_process: CirculantSymmetricMatrixDiffusionProcess,
    ) -> None:
        """device_ must equal torch.device('cpu') after CPU fit."""
        assert cpu_process.device_ == torch.device("cpu")

    def test_q_mat_is_on_cpu_after_cpu_fit(
        self,
        cpu_process: CirculantSymmetricMatrixDiffusionProcess,
    ) -> None:
    ...
    def test_diagonal_process_grids_are_on_cpu(
        self,
        cpu_process: CirculantSymmetricMatrixDiffusionProcess,
    ) -> None:
        """Both precomputed integral grids in diagonal_process must be on CPU.

        Tested with separate assertions so a failure identifies which
        integral path (D or F) dropped the device.
        """
        diag = cpu_process.diagonal_process_

        assert diag.precomputed_d_grid_.device.type == "cpu", (
            "precomputed_d_grid_ must be on cpu — check that "
            "_fft_to_eigenspace(x) preserves x.device before passing "
            "to DiagonalDiffusionProcess.fit()"
        )
        assert diag.precomputed_sigma_f_grid_.device.type == "cpu", (
            "precomputed_sigma_f_grid_ must be on cpu — check that "
            "_fft_to_eigenspace(x) preserves x.device before passing "
            "to DiagonalDiffusionProcess.fit()"
        )
    ...
    def test_mean_cond_output_device_is_cpu(
        ...
        """mean_cond output must be on CPU when fitted on CPU data."""
    ...
    def test_multiply_sigma_output_device_is_cpu(
        ...
        """multiply_sigma output must be on CPU when fitted on CPU data."""
    ...
    def test_diffusion_times_v_output_device_is_cpu(
        ...
        """diffusion_times_v output must be on CPU when fitted on CPU data."""
    ...
    def test_all_grids_are_on_cuda_after_cuda_fit(
        self,
        cuda_process: CirculantSymmetricMatrixDiffusionProcess,
    ) -> None:
        """Every precomputed tensor must be on CUDA after CUDA fit.

        Four attributes checked with individual failure messages to
        identify which link in the device-propagation chain failed.
        """
        diag = cuda_process.diagonal_process_

        assert cuda_process.device_.type == "cuda", (
            "self.device_ must be 'cuda' — check self.device = x.device"
            " in fit()"
        )
        assert cuda_process.q_mat_.device.type == "cuda", (
            "q_mat must be on cuda — check _get_cosine_basis device"
            " argument in fit()"
        )
        ...
        assert diag.precomputed_sigma_f_grid_.device.type == "cuda", (
            ...
            "_precompute_F_integral device routing in"
            " DiagonalDiffusionProcess"
        )
    ...
    def test_mean_cond_and_multiply_methods_output_device_is_cuda(
        ...
        """All operator outputs must be on CUDA when fitted on CUDA data."""
        torch.manual_seed(SEED)
        x_cuda = torch.randn(BATCH_SIZE, DATA_DIM, device="cuda")
        ...
        results = {
            ...
            "diffusion_times_v": cuda_process.diffusion_times_v(
                x_cuda, t_cuda,
            ),
        }
    ...
    def test_cross_device_query_raises_value_error(
        ...
        """CPU inputs on a CUDA-fitted process must raise ValueError.

        The diagonal process has an explicit device guard. This test
        verifies the circulant wrapper does not swallow or bypass it.
        """
        with pytest.raises(ValueError, match=r"device"):
