...
_RATIO_ATOL: float = 1e-6
_VE_COV_MIN_AT_T: float = 10.0
...
@pytest.fixture(scope="module")
def zero_drift() -> Callable[[torch.Tensor, torch.Tensor], torch.Tensor]:
    """f(x, t) = 0."""
    def _drift(
        x: torch.Tensor, _t: torch.Tensor,
    ) -> torch.Tensor:
...
@pytest.fixture(scope="module")
def identity_drift() -> Callable[[torch.Tensor, torch.Tensor], torch.Tensor]:
    """f(x, t) = x."""
    def _drift(
        x: torch.Tensor, _t: torch.Tensor,
    ) -> torch.Tensor:
...
@pytest.fixture(scope="module")
def scalar_diffusion() -> Callable[[torch.Tensor], torch.Tensor]:
...
@pytest.fixture(scope="module")
def diagonal_diffusion() -> Callable[[torch.Tensor], torch.Tensor]:
...
@pytest.fixture(scope="module")
def matrix_diffusion() -> Callable[[torch.Tensor], torch.Tensor]:
...
@pytest.fixture(scope="module")
def invalid_diffusion() -> Callable[[torch.Tensor], torch.Tensor]:
...
@pytest.fixture(scope="module")
def custom_process_scalar(
    zero_drift: Callable[[torch.Tensor, torch.Tensor], torch.Tensor],
    scalar_diffusion: Callable[[torch.Tensor], torch.Tensor],
) -> CustomDiffusionProcess:
    """CustomDiffusionProcess with scalar (ndim=1) diffusion."""
    return CustomDiffusionProcess(
        drift=zero_drift, diffusion=scalar_diffusion,
    )


@pytest.fixture(scope="module")
def custom_process_diagonal(
    zero_drift: Callable[[torch.Tensor, torch.Tensor], torch.Tensor],
    diagonal_diffusion: Callable[[torch.Tensor], torch.Tensor],
) -> CustomDiffusionProcess:
    """CustomDiffusionProcess with diagonal (ndim=2) diffusion."""
    return CustomDiffusionProcess(
        drift=zero_drift, diffusion=diagonal_diffusion,
    )


@pytest.fixture(scope="module")
def custom_process_matrix(
    zero_drift: Callable[[torch.Tensor, torch.Tensor], torch.Tensor],
    matrix_diffusion: Callable[[torch.Tensor], torch.Tensor],
) -> CustomDiffusionProcess:
    """CustomDiffusionProcess with full matrix (ndim=3) diffusion."""
    return CustomDiffusionProcess(
        drift=zero_drift, diffusion=matrix_diffusion,
    )
...
def vp_any_schedule(
    ...
    """Fitted VP parametrized over both schedules.

    Generates [linear]/[cosine] test IDs.
    """
...
class TestDiffusionTimesV:
    ...
    def test_output_shape_scalar(
        self,
        custom_process_scalar: CustomDiffusionProcess,
    ...
    def test_output_values_scalar(
        self,
        custom_process_scalar: CustomDiffusionProcess,
        v_batch: torch.Tensor,
        t_batch: torch.Tensor,
    ) -> None:
        """Row n must be scaled by diffusion_term[n] = t[n] across all dims."""
        result = custom_process_scalar.diffusion_times_v(v_batch, t_batch)
    ...
    def test_each_row_scaled_by_its_own_scalar(
        ...
        """Row n must be scaled by diffusion_term[n], not any other entry."""
        result = custom_process_scalar.diffusion_times_v(v_batch, t_batch)

        # Avoid division by zero: only check positions where v != 0
        nonzero_mask = v_batch.abs() > _RATIO_ATOL
        ratios = result[nonzero_mask] / v_batch[nonzero_mask]

        expected_ratios = (
            t_batch.unsqueeze(1).expand_as(v_batch)[nonzero_mask]
        )
        assert torch.allclose(ratios, expected_ratios, atol=_RATIO_ATOL)
    ...
    def test_output_shape_diagonal(
        self,
        custom_process_diagonal: CustomDiffusionProcess,
        v_batch: torch.Tensor,
        t_batch: torch.Tensor,
    ) -> None:
        """Output shape must match v."""
    ...
    def test_non_trivial_diagonal_scales_each_dimension_independently(
        ...
        """Each dim m is scaled by diffusion_term[n, m] independently."""
        n_dims = v_batch.shape[1]
        scale = torch.arange(1, n_dims + 1, dtype=torch.float32)
        ...
        proc = CustomDiffusionProcess(
            drift=zero_drift, diffusion=column_scaled_diffusion,
        )
    ...
    def test_zero_diagonal_gives_zero_output(
        self,
        zero_drift: Callable[[torch.Tensor, torch.Tensor], torch.Tensor],
        v_batch: torch.Tensor,
        t_batch: torch.Tensor,
        ...
        def zero_diagonal_diffusion(t: torch.Tensor) -> torch.Tensor:
            return torch.zeros(t.shape[0], n_dims)

        proc = CustomDiffusionProcess(
            drift=zero_drift, diffusion=zero_diagonal_diffusion,
        )
        result = proc.diffusion_times_v(v_batch, t_batch)

        assert torch.allclose(result, torch.zeros_like(v_batch))

    def test_output_shape_matrix(
        self,
        custom_process_matrix: CustomDiffusionProcess,
        v_batch: torch.Tensor,
        t_batch: torch.Tensor,
    ) -> None:
        """Output shape must match v."""
    ...
    def test_result_matches_torch_bmm(
        self,
        zero_drift: Callable[[torch.Tensor, torch.Tensor], torch.Tensor],
        v_batch: torch.Tensor,
        t_batch: torch.Tensor,
    ) -> None:
        """The einsum must match torch.bmm (with upper-triangular matrix)."""
        n_samples, n_dims = v_batch.shape
        fixed_matrix = torch.triu(torch.ones(n_dims, n_dims))

        def upper_tri_diffusion(t: torch.Tensor) -> torch.Tensor:
            return fixed_matrix.unsqueeze(0).expand(t.shape[0], -1, -1)

        proc = CustomDiffusionProcess(
            drift=zero_drift, diffusion=upper_tri_diffusion,
        )
        result = proc.diffusion_times_v(v_batch, t_batch)

        # Reference: batched matrix-vector product via torch.bmm
        g_expanded = fixed_matrix.unsqueeze(0).expand(n_samples, -1, -1)
    ...
    def test_per_sample_matrices_are_applied_independently(
        ...
        """Each sample n uses its own matrix.

        diffusion_term[n] = (n+1)*I → result[n] = (n+1)*v[n].
        """
        n_samples, n_dims = v_batch.shape

        def sample_scaled_identity(_t: torch.Tensor) -> torch.Tensor:
            scale = torch.arange(1, n_samples + 1, dtype=torch.float32)
            eye = torch.eye(n_dims)
            return scale.view(n_samples, 1, 1) * eye.unsqueeze(0)
        ...
        scale = (
            torch.arange(1, n_samples + 1, dtype=torch.float32).unsqueeze(1)
        )
    ...
    def test_zero_dim_scalar_tensor_raises_value_error(
        ...
        """
        def scalar_tensor_diffusion(_t: torch.Tensor) -> torch.Tensor:
            return torch.tensor(0.5)  # ndim=0

        proc = CustomDiffusionProcess(
            drift=zero_drift, diffusion=scalar_tensor_diffusion,
        )
    ...
    @pytest.mark.parametrize(
        "extra_dims", [1, 2, 3], ids=["ndim4", "ndim5", "ndim6"],
    )
    def test_any_ndim_above_3_raises(
        self,
        zero_drift: Callable[[torch.Tensor, torch.Tensor], torch.Tensor],
        v_batch: torch.Tensor,
        t_batch: torch.Tensor,
        ...
        def high_dim_diffusion(t: torch.Tensor) -> torch.Tensor:
            ...
            return torch.ones(t.shape[0], n_dims, n_dims, *extra_shape)
...
class TestVPFit(ForwardDiffusionFitTests, ForwardDiffusionCheckpointTests):
    ...
    def make_process(
        self,
    ) -> Callable[[], VariancePreservingDiffusionProcess]:
        """Return a factory for a fresh unfitted VP instance."""
    ...
    def test_checkpoint_round_trip(
        self,
        make_process: Callable[[], VariancePreservingDiffusionProcess],
    ) -> None:
...
class TestVEFit(ForwardDiffusionFitTests, ForwardDiffusionCheckpointTests):
    ...
    def make_process(
        self,
    ) -> Callable[[], VarianceExplodingDiffusionProcess]:
        """Return a factory for a fresh unfitted VE instance."""
    ...
    def test_checkpoint_round_trip(
        self,
        make_process: Callable[[], VarianceExplodingDiffusionProcess],
    ) -> None:
        """Full state must survive to_checkpoint() → from_checkpoint().
...
class TestVariancePreservingDiffusionProcessCov(ScalarCovarianceOperatorTests):
    ...
    def test_cov_at_t0_is_zero(
        self,
        process: VariancePreservingDiffusionProcess,
        t_zero: torch.Tensor,
    ) -> None:
    ...
    def test_cov_at_t_end_is_close_to_one(
        self,
        process: VariancePreservingDiffusionProcess,
        t_one: torch.Tensor,
    ) -> None:
        ...
        """
        result = process._cov(t_one)  # noqa: SLF001
    ...
    def test_variance_preserving_identity(
        ...
        mu_t  = process.mean_cond(x_ones, t_sweep)[:, 0]
        cov_t = process._cov(t_sweep)  # noqa: SLF001

        assert torch.allclose(
            mu_t ** 2 + cov_t, torch.ones_like(cov_t), atol=1e-6,
        )
...
class TestVarianceExplodingDiffusionProcessCov(ScalarCovarianceOperatorTests):
    ...
    @pytest.fixture(
        params=["linear", "exponential"], ids=["linear", "exponential"],
    )
    def process(
        self,
        request: pytest.FixtureRequest,
    ...
    def process_linear(
        ...
        ve = VarianceExplodingDiffusionProcess(
            g_schedule="linear", g_0=0.1, g_T=15.0,
        )
        ve.fit(x_batch)
        return ve
    ...
    def process_exp(
        ...
        """VE with exponential schedule; exponential oracle test only."""
        ve = VarianceExplodingDiffusionProcess(
    ...
    def test_cov_at_t0_is_zero(
        self,
        process: VarianceExplodingDiffusionProcess,
        t_zero: torch.Tensor,
    ) -> None:
        """_cov(0) must equal zero exactly for both schedules.
    ...
    def test_cov_is_monotonically_nondecreasing(
        self,
        process: VarianceExplodingDiffusionProcess,
        t_sweep: torch.Tensor,
    ) -> None:
        """_cov(t) must be non-decreasing: sigma_t² = ∫₀ᵗ g(s)² ds, g > 0."""
        cov_values = process._cov(t_sweep)  # noqa: SLF001
    ...
    def test_cov_at_t_end_is_large(
        self,
        process: VarianceExplodingDiffusionProcess,
        t_one: torch.Tensor,
    ) -> None:
        """_cov(T) must be >> 1 for both schedules (variance-exploding).
        ...
        assert torch.all(result > _VE_COV_MIN_AT_T), (
            f"_cov(T) too small for a variance-exploding process: "
            f"min found = {result.min().item():.4f}"
        )

    def test_linear_cov_matches_formula_at_known_points(
        self, process_linear: VarianceExplodingDiffusionProcess,
    ) -> None:
        """_cov(t) for the linear schedule must match hand-computed values.
        ...
        expected = torch.tensor(
            [0.000000, 1.251927, 9.627917, 32.065781, 75.503333],
        )

        result = process_linear._cov(t)  # noqa: SLF001

        assert torch.allclose(result, expected, atol=1e-4)

    def test_exponential_cov_matches_formula_at_known_points(
        self, process_exp: VarianceExplodingDiffusionProcess,
    ) -> None:
        """_cov(t) for the exponential schedule matches hand-computed values.
        ...
        atol=1e-3: limited precision of the hand-computed log constant.
        """
        t = torch.tensor([0.0, 0.25, 0.5, 0.75, 1.0])
        expected = torch.tensor(
        ...
        result = process_exp._cov(t)  # noqa: SLF001
...
class TestVPSampleLimit(ForwardDiffusionSampleLimitTests):
    ...
    def make_process(
        ...
        """Return a factory for a fresh unfitted VP instance (seeded)."""
        return lambda: VariancePreservingDiffusionProcess(
            beta_schedule="linear",
            beta_min=0.1,
            beta_max=10.0,
    ...
    def make_process_alt_seed(
        ...
        """Same VP but different seed; confirms genuine sampling variation."""
        return lambda: VariancePreservingDiffusionProcess(
            beta_schedule="linear",
            beta_min=0.1,
            beta_max=10.0,
    ...
    def test_limit_distribution_is_approximately_standard_normal(
        ...
        atol=0.05 is a ~5-sigma guard; P(false failure, M=16 dims) < 1e-5.
        """
        n = 10_000
...
class TestVESampleLimit(ForwardDiffusionSampleLimitTests):
    ...
    def make_process(
        ...
        """Return a factory for a fresh unfitted VE instance (seeded)."""
        return lambda: VarianceExplodingDiffusionProcess(
            g_schedule="exponential",
            g_0=0.1,
            g_T=15.0,
    ...
    def make_process_alt_seed(
        ...
        """Same VE but different seed; confirms genuine sampling variation."""
        return lambda: VarianceExplodingDiffusionProcess(
            g_schedule="exponential",
            g_0=0.1,
            g_T=15.0,
    ...
    def test_limit_distribution_statistics(
        ...
        sigma_t_end = torch.sqrt(
            fitted_process._cov(t_end),  # noqa: SLF001
        ).item()

        mean_tol = 5.0 * sigma_t_end / (n ** 0.5)
        ...
        rel_err = (
            (samples.std(dim=0) / sigma_t_end) - 1.0
        ).abs().max().item()
        assert rel_err < _STAT_ATOL, (
            f"Per-dimension std not close to sigma_T={sigma_t_end:.4f}: "
            f"max relative error = {rel_err:.4f}"
        )
...
class TestVarianceExplodingDiffusionProcessInit:
    ...
    def test_invalid_schedule_raises_error(self) -> None:
        ...
        with pytest.raises(ValueError, match="Unknown g schedule"):
    ...
    def test_nonpositive_bounds_raise_for_exponential_schedule(
        self, g_0: float, g_t: float,
    ) -> None:
...
class TestVariancePreservingDiffusionProcessMeanCond:
    ...
    def test_mean_cond_at_t0_equals_x(
        self,
        vp_any_schedule: VariancePreservingDiffusionProcess,
        x_batch: torch.Tensor,
        t_zero: torch.Tensor,
    ) -> None:
    ...
    def test_mean_cond_at_t_end_is_close_to_zero(
        self,
        vp_any_schedule: VariancePreservingDiffusionProcess,
        t_one: torch.Tensor,
    ) -> None:
...
class TestVarianceExplodingDiffusionProcessMeanCond:
    ...
    def test_mean_cond_at_t_end_equals_x(
        self,
        process: VarianceExplodingDiffusionProcess,
        x_batch: torch.Tensor,
        t_one: torch.Tensor,
    ) -> None:
        """mean_cond(x, T) must equal x; zero-drift VE never scales signal."""
        result = process.mean_cond(x_batch, t_one)

        assert torch.allclose(result, x_batch)

    def test_mean_cond_equals_x_for_interior_t(
        self,
        process: VarianceExplodingDiffusionProcess,
        x_batch: torch.Tensor,
        t_batch: torch.Tensor,
...
class TestVariancePreservingDiffusionProcessBetaT:
    ...
    All higher-level methods delegate time-dependent behavior here.
    """

    def test_linear_beta_t_at_t0_equals_beta_min(
        self,
        vp_linear: VariancePreservingDiffusionProcess,
        t_zero: torch.Tensor,
    ) -> None:
        """β(0) must equal beta_min exactly.

        β(t) = beta_min + (beta_max-beta_min)·t/T; second term = 0 at t=0.
        """
        result = vp_linear._beta_t(t_zero)  # noqa: SLF001

        expected = torch.full_like(t_zero, vp_linear.beta_min)
        assert torch.allclose(result, expected)

    def test_linear_beta_t_at_t_end_equals_beta_max(
        self,
        vp_linear: VariancePreservingDiffusionProcess,
        t_one: torch.Tensor,
    ) -> None:
        """β(T) must equal beta_max exactly."""
        result = vp_linear._beta_t(t_one)  # noqa: SLF001

        expected = torch.full_like(t_one, vp_linear.beta_max)
        assert torch.allclose(result, expected)

    def test_linear_beta_t_matches_formula_at_known_points(
        self,
        vp_linear: VariancePreservingDiffusionProcess,
    ) -> None:
        ...
        result = vp_linear._beta_t(t)  # noqa: SLF001
    ...
    def test_cosine_beta_t_is_clamped_between_min_and_max(
        self,
        vp_cosine: VariancePreservingDiffusionProcess,
        t_sweep: torch.Tensor,
    ) -> None:
        """β(t) (cosine schedule) must stay in [beta_min, beta_max].

        The cosine tangent grows unboundedly near t=T; torch.clamp keeps it in
        range. t_sweep exercises the interior, including high-curvature.
        """
        result = vp_cosine._beta_t(t_sweep)  # noqa: SLF001
    ...
    def test_beta_t_output_shape_matches_input(
        self,
        vp_any_schedule: VariancePreservingDiffusionProcess,
        t_batch: torch.Tensor,
    ) -> None:
        """Output shape must match t."""
        result = vp_any_schedule._beta_t(t_batch)  # noqa: SLF001
...
class TestVarianceExplodingDiffusionProcessDiffusion:
    ...
    def test_diffusion_at_t0_equals_g0(
        self,
        process: VarianceExplodingDiffusionProcess,
        t_zero: torch.Tensor,
    ) -> None:
    ...
    def test_diffusion_at_t_end_equals_g_t(
        self,
        process: VarianceExplodingDiffusionProcess,
        t_one: torch.Tensor,
    ) -> None:
    ...
    def test_diffusion_is_strictly_positive(
        self,
        process: VarianceExplodingDiffusionProcess,
        t_sweep: torch.Tensor,
    ) -> None:
        ...
        assert torch.all(result > 0), (
            f"g(t) not strictly positive: min={result.min().item():.6e}"
        )

    def test_diffusion_is_monotonically_increasing(
        self,
        process: VarianceExplodingDiffusionProcess,
        t_sweep: torch.Tensor,
    ) -> None:
    ...
    def test_linear_diffusion_matches_formula_at_known_points(
        self, process_linear: VarianceExplodingDiffusionProcess,
    ) -> None:
        """diffusion(t) must match hand-computed values (linear schedule).
    ...
    def test_exponential_diffusion_matches_formula_at_known_points(
        ...
        """diffusion(t) must match hand-computed values (exponential schedule).
        ...
        t        = torch.tensor([0.0, 0.25, 0.5, 0.75, 1.0])
        expected = torch.tensor(
            [0.10000, 0.34996, 1.22474, 4.28623, 15.00000],
        )
    ...
    def test_exponential_diffusion_with_equal_g0_g_t_is_constant(
        self,
        x_batch: torch.Tensor,
        t_batch: torch.Tensor,
    ) -> None:
        ...
        instead of g_0²·t); this test only covers diffusion().
        """
        g_const = 5.0
        ve = VarianceExplodingDiffusionProcess(
            g_schedule="exponential",
