...
class TestWeightedLp(unittest.TestCase):
    ...
    def test_lp_norm_grid(self) -> None:
        ...
        np.testing.assert_allclose(
            weighted_lp_norm(self.fd, p = 1),
            [16.0, 41.33333333],
        )

        np.testing.assert_allclose(
            weighted_lp_norm(self.fd, p = 2),
            [8.326664, 25.006666],
        )

        np.testing.assert_allclose(
            weighted_lp_norm(self.fd, p=3),
            [6.839904, 22.401268],
        )

        np.testing.assert_allclose(
            weighted_lp_norm(self.fd, p = math.inf),
            [6, 25],
        )
    ...
    def test_lp_norm_basis(self) -> None:
        ...
        np.testing.assert_allclose(
            weighted_lp_norm(self.fd_basis, p=2),
            [8.326664, 24.996],
        )
    ...
    def test_lp_norm_basis_equivalent(self) -> None:
        ...
        np.testing.assert_allclose(
            weighted_lp_norm(self.fd_grid_in_basis, p=1),
            [16.0, 41.33333333],
            rtol=1e-2,
        )

        np.testing.assert_allclose(
            weighted_lp_norm(self.fd_grid_in_basis, p=2),
            [8.326664, 25.006666],
            rtol=1e-2,
        )

        np.testing.assert_allclose(
            weighted_lp_norm(self.fd_grid_in_basis, p=3),
            [6.839904, 22.401268],
            rtol=1e-2,
        )
    ...
    def test_lp_norm_vector_valued(self) -> None:
        ...
        np.testing.assert_allclose(
            weighted_lp_norm(self.fd_vector_valued, p=1),
            [32.0, 82.666667],
        )
        np.testing.assert_allclose(
            weighted_lp_norm(self.fd_vector_valued, p=math.inf),
            [6, 25],
        )
    ...
    def test_lp_norm_surface_inf(self) -> None:
        ...
        np.testing.assert_allclose(
            weighted_lp_norm(self.fd_surface, p=np.inf).round(5),
    ...
    def test_lp_norm_surface(self) -> None:
        ...
        np.testing.assert_allclose(
            weighted_lp_norm(self.fd_surface, p=1),
    ...
    def test_lp_error_dimensions(self) -> None:
        ...
        with np.testing.assert_raises(ValueError):
            weighted_lp_distance(self.fd, self.fd_surface, p=2)

        with np.testing.assert_raises(ValueError):
            weighted_lp_distance(self.fd, self.fd_vector_valued, p=2)

        with np.testing.assert_raises(ValueError):
            weighted_lp_distance(self.fd_surface, self.fd_vector_valued, p=2)
    ...
    def test_lp_error_domain_ranges(self) -> None:
        ...
        with np.testing.assert_raises(ValueError):
            weighted_lp_distance(self.fd, fd2, p=2)
    ...
    def test_lp_array(self) -> None:
        ...
        np.testing.assert_allclose(
            weighted_lp_norm(self.array, p=1),
            [20, 55],
        )
        np.testing.assert_allclose(
            weighted_lp_norm(self.array,p=2),
            [9.48683298, 31.28897569],
        )

        np.testing.assert_allclose(
            weighted_lp_norm(self.array, p=3),
            [7.60590492, 27.37519199],
        )

        np.testing.assert_allclose(
            weighted_lp_norm(self.array, p=math.inf),
            [6, 25],
        )
