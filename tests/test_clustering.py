...
class TestClustering(unittest.TestCase):
    ...
    def test_kmeans_univariate(self):
        ...
        np.testing.assert_array_equal(kmeans.labels_,
                                      np.array([[0], [0], [0], [1]]))
        np.testing.assert_array_almost_equal(kmeans.cluster_centers_, np.array(
    ...
    def test_kmeans_multivariate(self):
        ...
        np.testing.assert_array_equal(kmeans.labels_,
                                      np.array([[1, 0],
                                                [1, 1],
                                                [0, 0]]))
        np.testing.assert_allclose(kmeans.cluster_centers_, np.array(
            [[[3., 4., 5., 6.],
              [1.5, 2.5, 3.5, 4.5]],
             [[0.25, 0.35, 0.45, 0.55],
              [0.5, 0.6, 0.7, 0.7]]]))
    ...
    def test_fuzzy_kmeans_univariate(self):
        ...
        fuzzy_kmeans.fit(fd)
        np.testing.assert_array_equal(fuzzy_kmeans.labels_,
                                      np.array([[[0.965, 0.035]],
                                                [[0.94, 0.06]],
                                                [[0.227, 0.773]],
                                                [[0.049, 0.951]]]))
        np.testing.assert_allclose(fuzzy_kmeans.cluster_centers_, np.array(
