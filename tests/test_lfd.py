...
class TestBasis(unittest.TestCase):
    ...
    def test_init_integer(self):
        ...
        np.testing.assert_equal(lfd_0.order, 0, "Wrong deriv order of the linear operator")
        np.testing.assert_equal(lfd_0.weights, weightfd, "Wrong list of weight functions of the linear operator")
        ...
        np.testing.assert_equal(lfd_3.order, 3, "Wrong deriv order of the linear operator")
        np.testing.assert_equal(lfd_3.weights, bwtlist3, "Wrong list of weight functions of the linear operator")
    ...
    def test_init_list_fdatabasis(self):
        ...
        np.testing.assert_raises(ValueError, Lfd, None, fdlist)

    def test_init_wrong_params(self):
        np.testing.assert_raises(ValueError, Lfd, 0, ['a'])
        np.testing.assert_raises(ValueError, Lfd, 0, 'a')
